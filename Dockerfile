...
RUN mkdir -p /workspace/models/diffusion_models && \
    aria2c -x 4 -s 4 -d /workspace/models/diffusion_models \
      ...
      "https://huggingface.co/city96/WAN2.2/resolve/main/wan2.2.safetensors" && \
    aria2c -x 4 -s 4 -d /workspace/models/diffusion_models \
      -o vace.safetensors \
      "https://huggingface.co/city96/VACE/resolve/main/vace.safetensors"
...
RUN mkdir -p /workspace/models/text_encoders && \
    aria2c -x 4 -s 4 -d /workspace/models/text_encoders \
      -o clip_text.pth \
      "https://huggingface.co/openai/clip-vit-large-patch14/resolve/main/clip_text.pth"

# CLIP vision encoder
RUN mkdir -p /workspace/models/clip_vision && \
    aria2c -x 4 -s 4 -d /workspace/models/clip_vision \
      -o clip_vision.pth \
      "https://huggingface.co/openai/clip-vit-large-patch14/resolve/main/clip_vision.pth"
